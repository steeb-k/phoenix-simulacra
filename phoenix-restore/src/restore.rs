...
/// Outcome of a successful restore. Distinguishes "restored at original
/// size" from "restored with one or more partitions resized" so the
/// caller (the GUI status bar, the CLI) can tell the user to run
/// `chkdsk /F` on the resized volumes — boot-sector patching makes the
/// volume mountable but `$Bitmap` / FAT / cluster-heap metadata still
/// describes the source layout and chkdsk's job is to reconcile the two.
